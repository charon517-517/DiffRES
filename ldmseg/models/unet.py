...
class DinoFuseAttention(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.selfattn = SelfAttnBlock(dim=1088, num_heads=8)
    def forward(self, sd_feat, dino_feat):
        b, c, h, w = sd_feat.shape
        dino_feat = nn.functional.interpolate(dino_feat, size=(h // 2, w // 2), mode='bilinear')
        sd_feat = nn.functional.interpolate(sd_feat, size=(h // 2, w // 2), mode='bilinear')
        fuse_feat = torch.cat([sd_feat, dino_feat], dim=1)
        fuse_feat = fuse_feat.reshape(b, -1, 1088)
        fuse_feat = self.selfattn(fuse_feat).reshape(b, -1, 32, 32)
        fuse_feat = nn.functional.interpolate(fuse_feat, scale_factor=2, mode='bilinear')
        return fuse_feat
...
class UNet(UNet2DConditionModel):
    ...
    def add_mask_decoder(self, in_channels=4):
        self.mask_decoder = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=1, kernel_size=3, padding=1, stride=1),
            nn.Sigmoid(),
        )
        # self.mask_decoder = nn.Identity()
    
    def add_dino_fuse_attn(self):
        self.dino_fuse_attn = DinoFuseAttention()

    def add_dino_condition_adapter(self):
        self.dino_adapter = TextAdapter()
    ...
    def add_text_condition_adapter(self, text_adapter_dim=768, adapter_type='simple'):
        if adapter_type == 'simple':
            self.text_adapter = TextAdapter(text_dim=text_adapter_dim)
        elif adapter_type == 'selfattn':
            self.text_adapter = SelfAttnAdapter(text_dim=text_adapter_dim)
        elif adapter_type == 'selfcross':
    ...
    def modify_encoder(
        ...
        assert in_channels in [4, 8, 776], "in_channels must be 4 or 8 or 776"
        ...
        # elif in_channels == 776:
        #     self.concat_text=True
        #     # get out_channels, kernelsize, stride, padding, bias
        #     out_channels, kernel_size = self.conv_in.out_channels, self.conv_in.kernel_size,
        #     stride, padding, bias = self.conv_in.stride, self.conv_in.padding, self.conv_in.bias
        #     self.new_conv = nn.Conv2d(in_channels+cond_channels, out_channels, kernel_size=kernel_size,
        ...
        #     # handle text part
        #     self.new_conv.weight.data[:, 8:776].zero_()  # text

        #     # handle bias
        #     self.new_conv.bias.data.copy_(self.conv_in.bias.data)
        #     # assert statements
        #     assert self.new_conv.weight.data.shape == torch.Size([320, 776 + cond_channels, 3, 3])
        #     if init_mode_seg == "copy":
        #         assert torch.all(self.new_conv.weight.data[:, :4] == self.conv_in.weight.data)
        #     if init_mode_image == "copy":
        #         assert torch.all(self.new_conv.weight.data[:, 4:8] == self.conv_in.weight.data)
        #     if cond_channels > 0:
        #         if init_mode_cond == "zero":
        #             self.new_conv.weight.data[:, 776:].zero_()
        #         elif init_mode_image == "mean":
        #             self.new_conv.weight.data[:, 776:].copy_(torch.mean(self.conv_in.weight.data, dim=1, keepdim=True).repeat(1, 4, 1, 1))  # noqa
        #         elif init_mode_cond == "random":
        #             pass
        #         else:
        #             raise NotImplementedError(f"init_mode cond {init_mode_cond} not implemented")
    ...
    def forward(
        ...
        if hasattr(self, 'text_adapter'):
            ...
            if _adaper_type in ['SelfCrossAttnAdapter', 'CrossAttnBlock']:
        ...
        # (optional) dino feature adapter for conditioning
        if hasattr(self, "dino_adapter"):
            dino_feat = self.dino_adapter(dino_feat) 
            encoder_hidden_states = torch.cat([encoder_hidden_states, dino_feat], dim=1)
        ...
        if hasattr(self, 'dino_fuse_attn'):
            assert dino_feat is not None
            sample = self.dino_fuse_attn(sample, dino_feat)
        else:
        ...
        if hasattr(self, 'mask_decoder'):
            # use mask decoder
            sample = self.mask_decoder(sample)
        else:
            # mean and sigmoid 
            sample = sample.mean(1, keepdim=True).sigmoid()

        if not return_dict:
            return (sample,)

        return UNetOutput(sample=sample, latent=latent)
...
class SelfAttnAdapter(nn.Module):
    def __init__(self, text_dim=768) -> None:
        super().__init__()
        self.attn_block = SelfAttnBlock(dim=text_dim, num_heads=8)
    def forward(self, text):
        return self.attn_block(text)
    
class SelfCrossAttnAdapter(nn.Module):
    def __init__(self, text_dim=768, vis_in=4, vis_dim=320) -> None:
        super().__init__()
        self.attn_block = SelfCrossAttnBlock(dim=text_dim, vis_in=vis_in, vis_dim=vis_dim, num_heads=8)
    def forward(self, text, vis_tokens):
        return self.attn_block(text, vis_tokens)