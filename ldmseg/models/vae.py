...
class SkipVAESeg(nn.Module):
    def __init__(self, down_scalar=4, scaling_factor=1.0, downsample_factor=8) -> None:
        super().__init__()
        self.scaling_factor = scaling_factor
    ...
    def encode(self, mask):
        ...
        latent_size = (h // self.downsample_factor, w // self.downsample_factor)
        latent = F.interpolate(mask, size=latent_size, mode="bilinear", align_corners=True)
        latent = SkipDist(latent)
        return EncoderOutput(latent_dist=latent)
    ...
    def decode(self, latent):
        ...
        latent = latent.repeat(1,2,1,1)
        latent[:, 0, :, :] = 1.0 - latent[:, 0, :, :]
        mask_size = (h * self.downsample_factor, w * self.downsample_factor)
        mask = F.interpolate(latent, size=(mask_size), mode='bilinear', align_corners=True)
        return mask
