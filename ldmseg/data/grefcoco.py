...
class GRefDataset(data.Dataset):
    def __init__(        
        ...
        fill_value: int = 0.5,
        ignore_label: int = 0,
        inpainting_strength: float = 0.0,) -> None:
        super().__init__()
        self.tokenizer = tokenizer
        self.data_dicts = load_grefcoco_json(prefix, dataset_name='grefcoco', splitby='unc', split=split ,image_root=os.path.join(prefix, 'images'))
    ...
    def __getitem__(self, index) -> Any:
        ...
        sample['image'] = img
        masks = []
        for mask in sample['annotations']:
            pass
...
if __name__ == "__main__":
    ...
    REFCOCO_PATH = '/root/autodl-tmp/datasets/refer_data/'
    COCO_TRAIN_2014_IMAGE_ROOT = '/root/autodl-tmp/datasets/refer_data/images'
    REFCOCO_DATASET = 'grefcoco'
    REFCOCO_SPLITBY = 'unc'
    REFCOCO_SPLIT = ['train', 'val', 'testA', 'testB']

    logger = setup_logger(name=__name__)

    for split in REFCOCO_SPLIT:
        dicts = load_grefcoco_json(REFCOCO_PATH, REFCOCO_DATASET, REFCOCO_SPLITBY, split, COCO_TRAIN_2014_IMAGE_ROOT)
