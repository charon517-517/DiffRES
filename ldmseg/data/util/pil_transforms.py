...
class Resize(object):
    ...
    def resize(self, img, h, w, mode='bicubic'):
        # resize
        if mode == 'bicubic':
            img = img.resize((w, h), resample=getattr(Image, 'Resampling', Image).BICUBIC, reducing_gap=None)
        elif mode == 'bilinear':
    ...
    def __call__(self, sample):
        for elem in sample.keys():
            if elem in ['image', 'image_semseg', 'semseg', 'mask', 'class_labels', 'image_class_labels']:
                sample[elem] = self.resize(sample[elem], self.size[0], self.size[1], mode=INT_MODES[elem])
        return sample
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            if 'meta' in elem or 'text' in elem or 'cat' in elem:
                continue
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            if 'meta' in elem or 'text' in elem or 'semseg' in elem or 'mask' in elem or 'cat' in elem:
                continue
