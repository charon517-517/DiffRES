...
class ReferDataset_Eval(data.Dataset):
    def __init__(
        self,
        ...
        self.split = split
        self.refer = REFER(prefix, refcoco_name, 'unc' if refcoco_name in ['refcoco', 'refcoco+'] else 'umd')
    ...
    def __getitem__(self, index):
        ...
        if self.eval_mode:
            ...
            tokens = []
            sentences = []
            for idx in range(len(self.input_ids[index])):
                tensor_embedding = self.input_ids[index][idx]
                raw_sentence = self.raw_sentences[index][idx]
                tokens.append(tensor_embedding)
