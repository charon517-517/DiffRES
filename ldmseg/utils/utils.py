...
except Exception:  # detectron2 is optional
    Visualizer = None
    _PanopticPrediction = None
    ColorMode = None
    _OFF_WHITE = (1.0, 1.0, 1.0)
    def _create_text_labels(*args, **kwargs):
        raise ImportError('detectron2 is required for visualization utilities')
...
# ---- Optional detectron2 visualization (disabled when detectron2 is not installed) ----
...
def collate_fn_test(batch: dict):
    # TODO: make general
    images = torch.stack([d['image'] for d in batch])
    semseg = torch.stack([d['semseg'] for d in batch])
    image_semseg = torch.stack([d['image_semseg'] for d in batch])
