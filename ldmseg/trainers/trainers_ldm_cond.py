...
try:
...
except Exception:  # torchvision is optional
    import os
    import numpy as _np
    ...
    def save_image(tensor, fp, **kwargs):
        ...
        if getattr(tensor, "ndim", None) == 4:
            tensor = tensor[0]
        if getattr(tensor, "ndim", None) == 3 and tensor.shape[0] in (1, 3):
            arr = tensor.float().cpu().clamp(0, 1)
            if arr.shape[0] == 1:
                arr = arr.repeat(3, 1, 1)
            arr = (arr.permute(1, 2, 0).numpy() * 255).astype(_np.uint8)
            _Image.fromarray(arr).save(fp)
        else:
...
from ldmseg.utils import (
    ...
    is_main_process, color_map, gpu_gather, collate_fn, collate_fn_test, 
...
class TrainerDiffusion(DatasetBase):
    ...
    def __init__(
        ...
        *,
        ...
        if self.vae_semseg.__class__.__name__ == "AutoEncoder":
            self.vae_semseg.downsample_factor = 8
        ...
        self.ds = self.get_dataset(
            ...
            refcoco_split=p['refcoco_split']
        )
        self.ds_val = self.get_dataset(
            p['val_db_name'],
            split=p.get('refcoco_split','val'),
    ...
    def compute_loss(
        ...
        if not self.no_noise_input:
            inputs = torch.cat([noisy_latents, rgb_latents], dim=1)
        else:
            inputs = rgb_latents
        #1aa. (optinonal) add text
        if hasattr(self.unet_model.module, 'concat_text'):
            concat_text = encoder_hidden_states.mean(1).unsqueeze(-1).unsqueeze(-1).repeat(1,1,64,64)
            inputs = torch.cat([inputs, concat_text], dim=1)
        ...
        # (optional) extract dino feature
        if hasattr(self, 'dino_backbone'):
            with torch.no_grad():
                dino_imgs = F.interpolate(rgb_imgs, size=(224, 224), mode='bilinear')
                dino_feat = self.dino_backbone(dino_imgs).last_hidden_state
        else:
            dino_feat = None
        ...
        # )
        # tgt_mask_logits = self.decode_latents(
        #     target,
        #     return_logits=True,
        #     threshold_output=False,
        #     rgb_latents=rgb_latents,
        ...
        #     masks_logits,
        #     size=(rgb_imgs.shape[-2], rgb_imgs.shape[-1]),
        #     mode="bilinear",
        #     align_corners=False,
        # )
        # tgt_mask_logits = F.interpolate(
        #     tgt_mask_logits,
        #     size=(rgb_imgs.shape[-2], rgb_imgs.shape[-1]),
        #     mode="bilinear",
        #     align_corners=False,
        # )
        # if not self.vae_semseg.__class__.__name__ == "AutoEncoder":
        #     miou_mask_pred = masks_logits.cpu().argmax(1).data.numpy()
        #     miou_mask_tgt = tgt_mask_logits.cpu().argmax(1).data.numpy()
        # else:
        #     # masks_logits_min = masks_logits.flatten(1).min(1)[0].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1,1,masks_logits.shape[-2], masks_logits.shape[-1])
        #     # masks_logits_max = masks_logits.flatten(1).max(1)[0].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1,1,masks_logits.shape[-2], masks_logits.shape[-1])
        #     miou_mask_pred = (masks_logits.cpu().squeeze(1) > 0.5).long().data.numpy()
        #     miou_mask_tgt = (tgt_mask_logits.cpu().squeeze(1) > 0.5).long().data.numpy()
        # miou_gt = data['semseg'].data.numpy()
        # I1 = np.sum(np.logical_and(miou_mask_pred, miou_gt))
        # U1 = np.sum(np.logical_or(miou_mask_pred, miou_gt))
        # print(f"pred iou = {I1 / U1}")
        # I2 = np.sum(np.logical_and(miou_mask_tgt, miou_gt))
        # U2 = np.sum(np.logical_or(miou_mask_tgt, miou_gt))
        # print(f"tgt iou = {I2 / U2}\n")
    ...
    def process_inputs(self, data: dict) -> ModelInputs:
        ...
        if not self.vae_semseg.__class__.__name__ == "AutoEncoder":
            ...
            with torch.no_grad():
                latents = self.vae_semseg.encode(images.to(torch.float32))
    ...
    def train_single_epoch(
        ...
        for batch_idx, data in enumerate(self.dl):
            ...
            if not self.one_step:
                ...
                noisy_latents = torch.randn_like(model_inputs.latents, device=model_inputs.latents.device, dtype=torch.float32)
            ...
            if self.self_condition:
                ...
                with torch.no_grad():
                    if hasattr(self.unet_model.module, 'concat_text'):
                        inputs = torch.cat([noisy_latents, model_inputs.rgb_latents, model_inputs.encoder_hidden_states.mean(1).unsqueeze(-1).unsqueeze(-1).repeat(1,1,64,64),condition], dim=1)
                    else:
            ...
            masks_logits = self.decode_latents(
                ...
                return_logits=True,
                threshold_output=False,
                rgb_latents=model_inputs.rgb_latents,
                weight_dtype=torch.float32,
            )
            masks_logits = F.interpolate(
                masks_logits,
                size=(model_inputs.rgb_images.shape[-2], model_inputs.rgb_images.shape[-1]),
                mode="bilinear",
                align_corners=False,
            )
            miou_mask_pred = masks_logits.cpu().argmax(1).data.numpy()
            miou_gt = data['semseg'].data.numpy()
            I = np.sum(np.logical_and(miou_mask_pred, miou_gt))
            U = np.sum(np.logical_or(miou_mask_pred, miou_gt))
            # print(self.unet_model.module.mid_block.attentions[0].proj_in.weight)
            this_iou = I / (U+1e-6)
            total_iter += 1
            total_iou += this_iou
            total_I += I
            ...
            if is_main_process() and batch_idx % 10 == 0:
                print(f"[Epoch {epoch}]: iter {batch_idx}/{len(self.dl)}, losses:{total_loss}", flush=True)
        ...
        print(f"[Epoch {epoch}]: train evaluation: mIoU {total_iou/total_iter}, oIoU {total_I/total_U}")
    ...
    def train_loop(self) -> None:
        ...
        eval_metric = self.p.get('eval_kwargs', {}).get('metrics', None)
        if eval_metric in [None, False, 'none', 'None', '']:
            evaluation_fn = None
            print('Skipping evaluation (eval_kwargs.metrics is None/none).')
        else:
            evaluation_fn = partial(
                self.compute_metrics,
                metrics=[eval_metric],
                dataloader=self.dl_val,
                threshold_output=self.p['eval_kwargs']['threshold_output'],
                save_images=True,
                seed=0,
                max_iter=max_iter,
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if epoch % 1 == 0:
                ...
                evaluation_fn(set_save_model=True, epoch=epoch)
    ...
    def one_step_sample(self,
        ...
        if self.image_descriptor_model is not None:
            ...
            descriptors = descriptors.view(descriptors.shape[0], descriptors.shape[1], -1).permute(0, 2, 1)
            encoder_hidden_states = descriptors.to(torch.float)
        if self.textencoder is not None:
        ...
        if rgb_latents is not None:
            if self.self_condition and not hasattr(self.unet_model.module, 'concat_text'):
                inputs = torch.cat([latent_model_input, rgb_latents, condition], dim=1)
            elif self.self_condition and hasattr(self.unet_model.module, 'concat_text'):
                concat_text = encoder_hidden_states.mean(1).unsqueeze(-1).unsqueeze(-1).repeat(1,1,64,64)
                # concat_text = torch.cat([concat_text] * multiplier)
                inputs = torch.cat([latent_model_input, rgb_latents, concat_text, condition], dim=1)
            elif self.no_noise_input:
                inputs = rgb_latents
            else:
        ...
        # if fuse dino feature
        if hasattr(self, 'dino_backbone'):
            dino_imgs = F.interpolate(rgb_images, size=(224, 224), mode='bilinear')
            dino_feat = self.dino_backbone(dino_imgs).last_hidden_state
        else:
            dino_feat = None
        with torch.autocast('cuda', enabled=self.fp16_scaler is not None):
        ...
        # latent_mask = latent_prior.mean(1, keepdim=True).sigmoid()
        # vis_latent_mask = (255*(F.interpolate(latent_mask, scale_factor=8, mode='bilinear') > 0.5)).float().squeeze(0).repeat(3,1,1).permute(1,2,0).contiguous().cpu().numpy().astype(np.uint8)
        # vis_pred_mask = (255*(F.interpolate(mask_latent_pred, scale_factor=8, mode='bilinear') > 0.5)).float().squeeze(0).repeat(3,1,1).permute(1,2,0).contiguous().cpu().numpy().astype(np.uint8)
        # vis_img = (255*(rgb_images - rgb_images.min()) / (rgb_images.max() - rgb_images.min())).squeeze(0).permute(1,2,0).contiguous().cpu().numpy().astype(np.uint8)
        # vis = Image.fromarray(np.concatenate([vis_img, vis_pred_mask, vis_latent_mask], axis=1))
        # vis.save('latent_mask_compare.jpg')
        
        
        return mask_latent_pred, origin_latent
    ...
    def compute_miou(
        ...
        # record for oIoU and mIoU
        per_node_cum_I = 0.0
        per_node_cum_U = 0.0
        per_node_iou = 0.0
        per_node_samples = 0.0
        per_node_p_5 = 0.0
        per_node_p_6 = 0.0
        per_node_p_7 = 0.0
        per_node_p_8 = 0.0
        per_node_p_9 = 0.0
        per_node_conn = 0.0
        per_node_gt_conn = 0.0
        per_node_p_50_cum_I = 0.0
        per_node_p_50_cum_U = 0.0
        per_node_p_50_iou = 0.0
        per_node_p_50_cnt = 0.0
        ...
        print("Compute mIoU meteric ...")
        if dataloader is None:
            dataloader = self.dl_val
        if hasattr(dataloader.dataset, 'ref_ids_cur_ptr'):
            # refresh eval pointer
            dataloader.dataset.ref_ids_cur_ptr = {}
        ...
        if max_iter is not None:
            print('Running mIoU eval on subset percentage {}/{} ...'.format(max_iter, len(dataloader)))
        _t_star = time.time()
        save_dict = {}
        for batch_idx, data in enumerate(dataloader):
            if is_main_process() and batch_idx % 100 == 0:
                _t_end = time.time()
                print(f"val iter[{batch_idx}/{len(dataloader)}]: cost {(_t_end-_t_star)/60} min", flush=True)
                _t_star = time.time()
            file_names = [x["image_file"] for x in data['meta']]
            img_names = file_names[0]
            ...
            text = data['text']
            cat = data['cat']
            for prompt, category in zip(text, [cat]*len(text)):
                # prompt = f"A photo of {category}, {prompt}"
                rgb_latents, _ = self.encode_inputs(
                    rgb_images,
                    encode_func=self.vae_image.encode,
                    scaling_factor=self.vae_image.scaling_factor,
                ...
                if not self.one_step:
                    # ddim sampling based denoising predict
                    ...
                    # directly predict mask latent from unet input
                    semseg_latents, origin_latent = self.one_step_sample(
                        prompts=[prompt],
                        rgb_latents=rgb_latents,
                        seed=seed,
                        rgb_images=rgb_images
                ...
                miou_mask_pred = masks_logits.cpu().argmax(1).data.numpy()
                miou_gt = data['semseg'].data.numpy()
                I, U = computeIoU(miou_mask_pred, miou_gt)
                if U == 0:
                    this_iou = 0.0
                else:
                    this_iou = I*1.0/U
                ...
                if  this_iou >= 0.7:
                    per_node_p_50_cum_I += I
                    per_node_p_50_cum_U += U
                    per_node_p_50_iou += this_iou
                    per_node_p_50_cnt += 1
                ...
                if 0.9 <= this_iou:
                    ...
                    per_node_p_8 += 1
                    per_node_p_9 += 1
                elif 0.8 <= this_iou < 0.9:
                    per_node_p_5 += 1
                    per_node_p_6 += 1
                    per_node_p_7 += 1
                    per_node_p_8 += 1
                elif 0.7 <= this_iou < 0.8:
                    per_node_p_5 += 1
                    per_node_p_6 += 1
                    per_node_p_7 += 1
                elif 0.6 <= this_iou < 0.7:
                    per_node_p_5 += 1
                    per_node_p_6 += 1
                elif 0.5 <= this_iou < 0.6:
                    per_node_p_5 += 1
            
                # save_idx = img_names.split('/')[-1].split('.')[0] + "_" + prompt.replace(" ", "_")
                # save_idx = save_idx.replace('/', "_")
                # save_root = "./diffres_refcoco_val_np_pred_mask"
                # if not os.path.exists(save_root):
                #     os.mkdir(save_root)
                # np.save(os.path.join(save_root, save_idx + ".npy"), np.squeeze(miou_mask_pred, axis=0).astype(np.uint8))
                # visulize
                # vis_img = ((rgb_images - rgb_images.min()) / (rgb_images.max() - rgb_images.min())) * 255
                # vis_img = vis_img.squeeze(0).cpu().numpy().astype(np.uint8).transpose(1,2,0)
                # vis_gt = (255 * miou_gt[0]).astype(np.uint8)
                # vis_pred = (255 * miou_mask_pred[0]).astype(np.uint8)
                # save_root = os.path.join("./vis_latent_mask", save_idx)
                # if not os.path.exists(save_root):
                #     os.makedirs(save_root)
                # Image.fromarray(vis_img).save(os.path.join(save_root, 'image.jpg'))
                # Image.fromarray(vis_gt).convert('L').save(os.path.join(save_root, 'gt.jpg'))
                # Image.fromarray(vis_pred).convert('L').save(os.path.join(save_root, 'pred.jpg'))
                # # visualize latent code
                # origin_latent = F.interpolate(origin_latent, scale_factor=8, mode='bilinear', align_corners=True)
                # origin_latent = (origin_latent.mean(1, keepdim=True).sigmoid() > 0.5).float().permute(1,0,2,3).cpu()
                # # origin_latent = F.interpolate(origin_latent, scale_factor=8, mode='bilinear', align_corners=False).permute(1,0,2,3).cpu()
                # save_image(origin_latent, os.path.join(save_root, 'latent_code.jpg'))
                
                # collect results
                per_node_cum_I += I
                per_node_cum_U += U
                per_node_iou += this_iou
                per_node_samples += 1
                ...
                #         align_corners=False
                #     )[0]  # [C, H, W]
                #     mask_pred = torch.argmax(mask_pred_result, dim=0)
                #     vis_pred = mask_pred.unsqueeze(0).repeat(3, 1, 1).permute(1, 2, 0)
                #     vis_pred = vis_pred.cpu().numpy()
                #     processed_results.append(mask_pred)
                #     vis_results.append(vis_pred)
                # for idx, (mask, ref) in enumerate(zip(vis_results, text)):
                #     _rnd = random.random()
                #     if _rnd > 0.01:
                #         continue
                #     mask = (mask * 255).astype(np.uint8)
                #     origin_img = np.array(Image.open(file_names[idx]).convert('RGB'))
                #     img_to_save = np.concatenate([origin_img, mask], axis=1)
                #     img_name = f"epoch_{epoch}_"+ref.replace(' ', '_')+".jpg"
                #     save_path = os.path.join(self.results_folder.resolve(), img_name)
                #     img_to_save = Image.fromarray(img_to_save)
                #     img_to_save.save(save_path)
        # gather all results from multiple gpus
        # with open("diffRES_results.json", 'w', encoding='utf-8') as f:
        #     print("save results")
        #     json.dump(save_dict, f)
        
        per_node_result = torch.tensor([per_node_samples, per_node_iou, per_node_cum_I, per_node_cum_U, per_node_p_5, per_node_p_6, per_node_p_7, per_node_p_8, per_node_p_9, per_node_conn, per_node_gt_conn, per_node_p_50_cum_I, per_node_p_50_cum_U, per_node_p_50_iou, per_node_p_50_cnt], device=self.args['gpu'])
            
        torch.cuda.synchronize()
        dist.all_reduce(per_node_result)
            
        if is_main_process():
            save_miou = per_node_result[1] / per_node_result[0]
            save_oiou = per_node_result[2] / per_node_result[3]
            p_5 = per_node_result[4] / per_node_result[0]
            p_6 = per_node_result[5] / per_node_result[0]
            p_7 = per_node_result[6] / per_node_result[0]
            p_8 = per_node_result[7] / per_node_result[0]
            p_9 = per_node_result[8] / per_node_result[0]
            # avg_conn = per_node_result[9] / per_node_result[0]
            # gt_avg_conn = per_node_result[10] / per_node_result[0]
            p_50_oiou = per_node_result[11] / per_node_result[12]
            p_50_miou = per_node_result[13] / per_node_result[14]
            
            print(f"Eval on {per_node_result[0]} samples", flush=True)
            print(f"mIoU={save_miou}", flush=True)
            print(f"oIoU={save_oiou}", flush=True)
            print(f"P>50 oIoU={p_50_oiou}", flush=True)
            print(f"P>50 mIoU={p_50_miou}", flush=True)
            # print(f"avg conn: {avg_conn}, gt conn: {gt_avg_conn}", flush=True)
            print(f"P@50: {p_5}\n P@60: {p_6} \nP@70: {p_7} \nP@80: {p_8} \nP@90: {p_9} \n", flush=True)
            if save_oiou > self.best_iou:
                print(f"update best, epoch:{epoch}, oIoU={save_oiou}, mIoU={save_miou}", flush=True)
                self.best_iou = save_oiou
    ...
    def sample(
        ...
        for sample_idx, t in tqdm(enumerate(scheduler.timesteps), disable=disable_progress_bar):
            ...
            if self.self_condition:
                ...
                condition = torch.cat([condition] * multiplier)
    ...
    @torch.no_grad()
    def sample_sigle_img(self, prompt="",img_path="/nfs_data/f6bbdd8f-56e2-4099-977a-37c86d30b5bc/datasets/refer_data/images/mscoco/images/train2014/COCO_train2014_000000000625.jpg"):
            from ldmseg.data.util import pil_transforms as pil_tr
            import torchvision.transforms as T
            transforms = T.Compose([
                pil_tr.CropResize((512, 512), crop_mode=None),
                pil_tr.ToTensor(),
                nn.Identity()
            ])
            rgb_img = Image.open(img_path).convert('RGB')
            rgb_img = transforms(rgb_img).unsqueeze(0)
            text = self.tokenizer(prompt)['input_ids']
            ...
            masks_logits = F.interpolate(
                masks_logits,
                size=(rgb_img.shape[-2], rgb_img.shape[-1]),
                mode="bilinear",
                align_corners=False,
            )
    ...
    def compute_pq(
        ...
        try:
            ...
            print(f'[WARN] PQ evaluator not available (ldmseg.evaluations missing). Skipping PQ. Error: {e}')
            return {}
    ...
    def resume(self, load_vae: bool = True) -> None:
        ...
        model_path = str(self.results_folder / 'best_model.pt')
        ...
        if load_vae:
            ...
            if not self.vae_semseg.__class__.__name__ == "AutoEncoder":
                ...
                del data['optimizer']
                self.vae_semseg.load_state_dict(data['model'])
