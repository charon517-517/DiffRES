...
def get_optim_unet(
    ...
    # # only fintune and conv_in
    # for module_name, _ in model.named_parameters():
    #     if 'conv_in' not in module_name and 'mask_decoder' not in module_name and 'text_adapter' not in module_name and 'down_blocks' not in module_name:
    #         print(module_name)
    #         overrides[module_name] = {"lr":0.0, "weight_decay":0.0}

    ret_ = get_optimizer_params(
        model, weight_decay=None, weight_decay_norm=weight_decay_norm, base_lr=base_lr,
        lr_factor_func=lr_factor_func,
        overrides=overrides,
        verbose=verbose,
    ...
    ret = []
    # only finetune conv_in
    for param_group in ret_:
        if param_group['lr'] != 0.0:
            ret.append(param_group)
