...
from ldmseg.models import UNet, GeneralVAESeg, GeneralVAEImage, SkipVAESeg, get_image_descriptor_model
...
def main_worker(
    ...
    # Ensure tokenizer/text_encoder exist for RefCOCO-style text inputs
    if tokenizer is None or text_encoder is None:
        try:
            from transformers import CLIPTokenizer, CLIPTextModel
            if tokenizer is None:
                tokenizer = CLIPTokenizer.from_pretrained(pretrained_model_path, subfolder='tokenizer', cache_dir=cache_dir)
            if text_encoder is None:
                text_encoder = CLIPTextModel.from_pretrained(pretrained_model_path, subfolder='text_encoder', cache_dir=cache_dir)
        except Exception as e:
            raise RuntimeError('Failed to auto-load CLIP tokenizer/text_encoder. Please install transformers and provide SD1.5 tokenizer/text_encoder weights.') from e
    ...
    if p['train_kwargs']['text_adapter']:
        print("use text adapter for text condition")
        unet.add_text_condition_adapter(adapter_type=p['train_kwargs']['adapter_type'])
        print(unet.text_adapter)

    if p['noise_scheduler_kwargs']['prediction_type'] == "sample":
        in_channels = 4
        unet.add_mask_decoder(in_channels)
        print("use mask deocder")
        print(unet.mask_decoder)
    ...
    # trainer.sample_sigle_img()
    # resume training from checkpoint
    trainer.resume(load_vae=False)  # looks for model at run_idx (automatically determined)
    trainer.load(model_path=p['load_path'], load_vae=p['load_vae'])  # looks for model at load_path
